...
class Createl():
    def __init__(self,master):
        ...
        self.file_menu.add_command(label = "Clear",accelerator = 'Control+C', command = self.clearfun)
        self.file_menu.add_command(label = "Ok" ,accelerator = 'Control+O', command = self.okb)
        self.file_menu.add_command(label="Exit",accelerator= 'Alt+F4',command = self.exitmenu)
        self.menu.add_cascade(label = "File",menu=self.file_menu)
